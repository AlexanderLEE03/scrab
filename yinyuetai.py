...
def getcontent(url,filepath):
    ...
    movie_list = soup.find('ul', class_ = 'area_three area_list')
    ...
    for li in movies:
        comment = {}
        try:
            # info = li.find('div',class_ = 'mv_info')
            # comment['title'] = li.find('div',class_ = 'listshare')['data-videoname']
            comment['title'] = li.find('a', class_='mvname').text
            comment['link'] = li.find('a', class_='mvname')['href']
            comment['img'] = baseurl+ li.find('img')['src']
            comment['name'] = li.find('a',class_ = 'special').text
            # comment['video'] = li.find('a',class_='mvname').text
            title = comment['title']
